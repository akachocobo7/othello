================================================================================
    MFC (MICROSOFT FOUNDATION CLASS) ライブラリ : osero プロジェクトの概要
===============================================================================

この osero アプリケーションは、アプリケーション ウィザードによって作成されました。このアプリケーションは MFC の基本的な使い方を示すだけでなく、アプリケーションを作成するための開始点でもあります。
...
osero.h
    ...
    その他のプロジェクト固有のヘッダー (Resource.h など) が含まれ、CoseroApp アプリケーション クラスを宣言します。

osero.cpp
    これはメインのアプリケーション ソース ファイルで、CoseroApp アプリケーション クラスが含まれています。
...
res\osero.ico
    これはアプリケーションのアイコンとして使用されるアイコン ファイルです。このアイコンは、メイン リソース ファイル osero.rc にインクルードされます。
...
res\Toolbar.bmp
    ...
    初期ツール バーと初期ステータス バーは、CMainFrame クラスで生成されます。ツール バーのボタンを追加するには、リソース エディターを使用してこのツール バーのビットマップを編集し、osero.rc 内の IDR_MAINFRAME TOOLBAR 配列を更新します。
/////////////////////////////////////////////////////////////////////////////

アプリケーション ウィザードは 1 つのドキュメントの種類と 1 つのビューを作成します。

oseroDoc.h, oseroDoc.cpp - ドキュメント
    これらのファイルには、CoseroDoc クラスが含まれます。これらのファイルを編集して、特殊なドキュメント データを追加し、ファイルの保存と読み込みを実装することができます (CoseroDoc::Serialize を使用)。

oseroView.h, oseroView.cpp - ドキュメントのビュー
    これらのファイルには、CoseroView クラスが含まれます。
    CoseroView オブジェクトは CoseroDoc オブジェクトを表示するために使用されます。
